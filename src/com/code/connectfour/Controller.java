...
public class Controller implements Initializable {
   ...
   private boolean isAllowedToInsert = true;  // to avoid same color discs being added.
    	...
    	for(int col=0 ; col<COLUMNS ; col++){
		    ...
		    rectangle.setOnMouseClicked(event -> {
		    	if(isAllowedToInsert) {
                    isAllowedToInsert = false;  // when the disc is being being dropped then no more disc will be inserted
    ...
    private void InsertDisc(Disc disc, int column){
          ...
          if (row < 0)  //if it is null ,we cannot insert anymore disc
          	return;
	...
	private boolean checkCombinations(List<Point2D> points) {
    	...
    	for(Point2D point: points ){
    		...
    		if(disc != null && disc.isPlyerOneMove == isPlayerOneTurn){   //if the last inserted disc belongs to the current player
    			chain++;
    			if(chain == 4){
    				return true;
			    }
	...
	private static class Disc extends Circle{
    	...
    	private final boolean isPlyerOneMove;

    	public Disc(boolean isPlyerOneMove){

    		this.isPlyerOneMove = isPlyerOneMove;
    		setRadius(CIRCLE_DIAMETER/2);
    		setFill(isPlyerOneMove? Color.valueOf(discColor1): Color.valueOf(discColor2));
	...
	public void resetGame() {
		...
		for(int row =0 ; row < insertedDiscsArray.length; row++){  // structurally make all the elements of insertedDiscsArray[][] to null
		...
		isPlayerOneTurn = true;  // let the playerone to start to game
